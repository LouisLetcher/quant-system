...
def main():
    ...
    # Core imports
    tests = [
        (lambda: check_import("src.core.data_manager", "Core data manager import"), None),
        (lambda: check_import("src.core.backtest_engine", "Core backtest engine import"), None),
        (lambda: check_import("src.cli.unified_cli", "CLI module import"), None),
        (lambda: run_command("python -m src.cli.unified_cli --help", "CLI help command"), None),
        (lambda: run_command("python -m src.cli.unified_cli cache stats", "Cache stats command"), None),
        (lambda: run_command(
