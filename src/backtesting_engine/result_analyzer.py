...
class BacktestResultAnalyzer:
    ...
    def analyze(backtest_result, ticker=None, initial_capital=10000):
        ...
        logger.info(
            f"Analyzing backtest results for {ticker if ticker else 'unknown asset'}"
        )
        ...
        if trades_list:
            ...
            logger.info(
                f"  Winning trades: {win_count} ({win_count/len(trades_list)*100 if trades_list else 0:.2f}%)"
            )
            logger.info(
                f"  Losing trades: {loss_count} ({loss_count/len(trades_list)*100 if trades_list else 0:.2f}%)"
            )

            if winning_trades:
                avg_win_amount = sum(t.get("pnl", 0) for t in winning_trades) / len(
                    winning_trades
                )
            ...
            if losing_trades:
                avg_loss_amount = sum(t.get("pnl", 0) for t in losing_trades) / len(
                    losing_trades
                )
            ...
            if ticker:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not analyze trade distribution by period: {e}"
                    )
        else:
        ...
        if ticker:
            ...
            if equity_curve and len(equity_curve) > 100:
                ...
                logger.info(
                    f"Sampled equity curve from {len(equity_curve)} to {len(serializable_results['equity_curve'])} points for serialization"
                )
            ...
            if drawdown_curve and len(drawdown_curve) > 100:
                ...
                logger.info(
                    f"Sampled drawdown curve from {len(drawdown_curve)} to {len(serializable_results['drawdown_curve'])} points for serialization"
                )
            ...
            if trades_list and len(trades_list) > 1000:
                ...
                logger.info(
                    f"Limited trades list from {len(trades_list)} to 1000 for serialization"
                )
    ...
    def _extract_equity_curve(results):
        ...
        try:
            ...
            if isinstance(equity_data, pd.DataFrame):
                logger.debug(
                    f"Processing DataFrame equity curve with {len(equity_data)} rows"
                )
                ...
                if "Equity" in equity_data.columns:
                    ...
                    logger.debug(
                        f"Equity column not found. Available columns: {equity_data.columns}"
                    )
                ...
                logger.debug(
                    f"Processing Series equity curve with {len(equity_data)} elements"
                )
    ...
    def _extract_drawdown_curve(results):
        ...
        try:
            ...
            if isinstance(equity_data, pd.DataFrame):
                logger.debug(
                    f"Processing DataFrame for drawdown with {len(equity_data)} rows"
                )

                # Check if 'DrawdownPct' column exists
                if "DrawdownPct" in equity_data.columns:
                    for date, value in zip(
                        equity_data.index, equity_data["DrawdownPct"]
                    ):
                ...
                # Handle case where equity curve is a Series
                logger.debug(
                    f"Processing Series for drawdown with {len(equity_data)} elements"
                )
            ...
            if drawdown_curve:
                ...
                if min_value > 0:
                    logger.warning(
                        f"Positive drawdown values detected: minimum = {min_value}%"
                    )
                if max_value < -100:
                    logger.warning(
                        f"Extreme drawdown values detected: maximum = {max_value}%"
                    )
    ...
    def _extract_trades_list(results):
        ...
        if (
            "_trades" not in results
            or results["_trades"] is None
            or results["_trades"].empty
        ):
        ...
        try:
            ...
            missing_columns = [
                col for col in column_mapping if col not in trades_df.columns
            ]
            if missing_columns:
                logger.warning(
                    f"Missing expected columns in trades data: {missing_columns}"
                )
            ...
            for _, trade in trades_df.iterrows():
                ...
                for orig_col, new_col in column_mapping.items():
                    if orig_col in trade:
                        try:
                            ...
                            logger.warning(
                                f"Error processing trade column {orig_col}: {e}"
                            )
                ...
                if (
                    "duration" not in trade_dict
                    and "entry_date" in trade_dict
                    and "exit_date" in trade_dict
                ):
                    try:
            ...
            if trades_list:
                ...
                avg_return = sum(
                    trade.get("return_pct", 0) for trade in trades_list
                ) / len(trades_list)
                ...
                if "Return [%]" in results and abs(results.get("Return [%]", 0)) > 0.01:
                    ...
                    if abs(strategy_return - total_return) > max(
                        5, strategy_return * 0.1
                    ):
                        logger.warning(
                            f"Discrepancy between strategy return ({strategy_return}%) and sum of trade returns ({total_return}%)"
                        )
