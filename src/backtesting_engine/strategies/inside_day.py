...
class InsideDayStrategy(BaseStrategy):
    ...
    def next(self):
        ...
        inside_day = (self.highs[-1] < self.highs[-2]) and (
        ...
        if not self.position and inside_day:
            ...
            print(
                f"📊 Inside Day Pattern: Yesterday's Range [{self.lows[-2]}-{self.highs[-2]}], Today's Range [{self.lows[-1]}-{self.highs[-1]}]"
            )
        ...
        # Exit logic: Close position when RSI is overbought
            ...
            print(
                f"📈 RSI: {self.rsi[-1]}, Overbought threshold: {self.overbought_level}"
            )
            self.position.close()
