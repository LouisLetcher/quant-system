...
class StrategyRunner:
    ...
    def execute(strategy_name, ticker, period="max", start=None, end=None, commission=0.001, initial_capital=10000, take_profit=None, stop_loss=None):
        ...
        engine = BacktestEngine(SizeLimitedStrategy, data, ticker=ticker, commission=commission, cash=initial_capital)
    ...
    def execute_portfolio(strategy_name, portfolio_name, portfolio_config, start=None, end=None):
        ...
        class SizeLimitedStrategy(strategy_class):
            def init(self):
                # Store initial capital for reference
                self._initial_capital = initial_capital / len(portfolio_data)  # Per asset
                # Call the original init method
                super().init()
