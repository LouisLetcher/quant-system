...
def backtest_all_strategies(args):
    ...
    print(
        f"Testing all strategies on {args.ticker} with {initial_capital} initial capital"
    )
    ...
    try:
        ...
        for strategy_name in strategies:
            ...
            try:
                ...
                all_results[strategy_name] = {"score": score, "results": results}
                ...
                print(f"    ❌ Error testing {strategy_name}: {e!s}")
                continue

        if best_strategy:
            print(
                f"✅ Best strategy for {args.ticker}: {best_strategy} ({args.metric.capitalize()}: {best_score})"
            )
        ...
        print(f"❌ Error running backtest: {e!s}")
        return {}
...
def backtest_interval(args):
    ...
    print(
        f"Testing {args.strategy} on {args.ticker} across intervals: {', '.join(intervals)}"
    )
    ...
    for interval in intervals:
        try:
            ...
            # Print key metrics
            print(
                f"  Return: {result.get('Return [%]', result.get('return_pct', 0)):.2f}%"
            )
            print(
                f"  Profit Factor: {result.get('Profit Factor', result.get('profit_factor', 0)):.2f}"
            )
            print(
                f"  Sharpe: {result.get('Sharpe Ratio', result.get('sharpe_ratio', 0)):.2f}"
            )
            print(f"  # Trades: {result.get('# Trades', 0)}")
        except Exception as e:
            print(f"  ❌ Error with interval {interval}: {e!s}")
...
def register_commands(subparsers):
    ...
    backtest_parser = subparsers.add_parser(
        "backtest", help="Backtest a single asset with a specific strategy"
    )
    ...
    all_strategies_parser = subparsers.add_parser(
        "all-strategies", help="Backtest a single asset with all strategies"
    )
    all_strategies_parser.add_argument(
        "--ticker", type=str, required=True, help="Stock ticker symbol"
    )
    all_strategies_parser.add_argument(
        "--period",
    ...
    all_strategies_parser.add_argument(
        "--commission", type=float, help="Commission rate"
    )
    all_strategies_parser.set_defaults(func=backtest_all_strategies)

    # Intervals command
    interval_parser = subparsers.add_parser(
        "intervals", help="Backtest a strategy across multiple bar intervals"
    )
    ...
    interval_parser.add_argument(
        "--period", type=str, default="max", help="How far back to test"
    )
    interval_parser.add_argument(
        "--initial-capital", type=float, help="Initial capital"
    )
    ...
    interval_parser.add_argument(
        "--intervals", type=str, nargs="+", default=None, help="Bar intervals to test"
    )
