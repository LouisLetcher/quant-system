...
def optimize_strategy(args):
    ...
    commission = args.commission if args.commission is not None else (
        asset_config.get('commission', defaults['commission']) if asset_config else defaults['commission']
    )
    initial_capital = args.initial_capital if args.initial_capital is not None else (
        asset_config.get('initial_capital', defaults['initial_capital']) if asset_config else defaults['initial_capital']
    )
    ...
    # Load data
    data = DataLoader.load_data(args.ticker, period=args.period, start=args.start_date, end=args.end_date)
    ...
    for attr_name in dir(strategy_class):
        ...
        if isinstance(attr_value, (int, float)):
            if attr_name.endswith('_period') or attr_name.endswith('_length'):
                ...
                param_ranges[attr_name] = (max(0.01, attr_value / 2), min(0.99, attr_value * 2))
            else:
...
def register_commands(subparsers):
    ...
    optimize_parser = subparsers.add_parser("optimize", 
                                         help="Optimize parameters for a strategy")
    ...
    optimize_parser.add_argument("--iterations", type=int, default=50, help="Number of optimization iterations")
    optimize_parser.add_argument("--initial-capital", type=float, help="Initial capital")
