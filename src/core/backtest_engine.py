...
class UnifiedBacktestEngine:
    ...
    def run_backtest(self, symbol: str, strategy: str, config: BacktestConfig,
                    custom_parameters: Dict[str, Any] = None) -> BacktestResult:
        """
        ...
        try:
            ...
            if config.use_cache and not custom_parameters:
                ...
                if cached_result:
                    ...
                    return self._dict_to_result(cached_result, symbol, strategy, parameters, config)
            ...
            if config.futures_mode:
                data = self.data_manager.get_crypto_futures_data(
                    symbol, config.start_date, config.end_date, config.interval, config.use_cache
                )
            else:
                data = self.data_manager.get_data(
                    symbol, config.start_date, config.end_date, config.interval, 
                    config.use_cache, config.asset_type
                )
            
            if data is None or data.empty:
                return BacktestResult(
                    symbol=symbol, strategy=strategy, parameters=parameters,
                    config=config, metrics={}, error="No data available"
                )
            ...
            return BacktestResult(
                ...
                config=config, metrics={}, error=str(e),
    ...
    def run_batch_backtests(self, config: BacktestConfig) -> List[BacktestResult]:
        ...
        combinations = [
            (symbol, strategy) for symbol in config.symbols 
        ...
        for i in range(0, len(combinations), batch_size):
            ...
            self.logger.info(f"Processing batch {i//batch_size + 1}/{(len(combinations)-1)//batch_size + 1}")
    ...
    def run_portfolio_backtest(self, config: BacktestConfig, 
        ...
        try:
            ...
            all_data = self.data_manager.get_batch_data(
                ...
                config.interval, config.use_cache, config.asset_type
            )
            
            if not all_data:
                return BacktestResult(
                    symbol="PORTFOLIO", strategy=strategy, parameters={},
                    config=config, metrics={}, error="No data available for any symbol"
                )
    ...
    def _execute_backtest(self, symbol: str, strategy: str, data: pd.DataFrame,
                         parameters: Dict[str, Any], config: BacktestConfig) -> BacktestResult:
        ...
        try:
            ...
            if not strategy_class:
                return BacktestResult(
                    symbol=symbol, strategy=strategy, parameters=parameters,
                    config=config, metrics={}, error=f"Strategy {strategy} not found"
                )
            ...
            # Analyze results
            metrics = self.result_analyzer.calculate_metrics(result, config.initial_capital)
            ...
            return BacktestResult(
                ...
                config=config,
                metrics=metrics,
                equity_curve=result.get('equity_curve') if config.save_equity_curve else None,
    ...
    def _execute_portfolio_backtest(self, data_dict: Dict[str, pd.DataFrame],
                                   strategy: str, weights: Dict[str, float],
        ...
        try:
            ...
            if aligned_data.empty:
                return BacktestResult(
                    symbol="PORTFOLIO", strategy=strategy, parameters=weights,
                    config=config, metrics={}, error="No aligned data for portfolio"
                )
            ...
            return BacktestResult(
                ...
                config=config,
                metrics=metrics,
                equity_curve=equity_curve.to_frame('equity') if config.save_equity_curve else None
            )
    ...
    def _process_batch(self, batch: List[Tuple[str, str]], 
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(self._run_single_backtest_task, symbol, strategy, config): (symbol, strategy)
            ...
            for future in concurrent.futures.as_completed(futures):
                ...
                try:
                    ...
                    self.logger.error(f"Batch backtest failed for {symbol}/{strategy}: {e}")
                    self.stats['errors'] += 1
                    results.append(BacktestResult(
                        symbol=symbol, strategy=strategy, parameters={},
                        config=config, metrics={}, error=str(e)
                    ))
            
            return results
    
    def _run_single_backtest_task(self, symbol: str, strategy: str, 
                                 config: BacktestConfig) -> BacktestResult:
    ...
    def _prepare_data_with_indicators(self, data: pd.DataFrame, 
                                     strategy_instance) -> pd.DataFrame:
    ...
    def _simulate_trading(self, data: pd.DataFrame, strategy_instance, 
        ...
        for i, (timestamp, row) in enumerate(data.iterrows()):
            ...
            if signal == 1 and position <= 0:  # Buy signal
                ...
                position = row['close']
                capital -= position_size * row['close'] + (position_size * row['close'] * config.commission)
        ...
        return {
            ...
            'final_capital': equity_curve[-1]['equity'] if equity_curve else config.initial_capital
        }
    ...
    def _calculate_portfolio_returns(self, aligned_data: pd.DataFrame, 
        ...
        for i in range(1, len(aligned_data)):
            ...
            for symbol, weight in weights.items():
                ...
                if col_name in aligned_data.columns:
                    asset_return = (aligned_data[col_name].iloc[i] / aligned_data[col_name].iloc[i-1]) - 1
    ...
    @staticmethod
    # @jit(nopython=True)  # Removed for compatibility
    def _calculate_macd(prices: np.ndarray, fast: int = 12, slow: int = 26, 
    ...
    def _dict_to_result(self, cached_dict: Dict, symbol: str, strategy: str,
                       parameters: Dict, config: BacktestConfig) -> BacktestResult:
