...
class UnifiedCacheManager:
    ...
    def cache_data(self, symbol: str, data: pd.DataFrame, interval: str = "1d", 
                   source: str = None, data_type: str = None, ttl_hours: int = 48) -> str:
        """
        Cache market data.
    ...
    def get_data(self, symbol: str, start_date: str = None, end_date: str = None,
                 interval: str = "1d", source: str = None, data_type: str = None) -> Optional[pd.DataFrame]:
        """
        ...
        with self.lock:
            ...
            entries = self._find_entries("data", symbol=symbol, interval=interval, 
                                       source=source, data_type=data_type)
    ...
    def cache_backtest_result(self, symbol: str, strategy: str, parameters: Dict[str, Any],
        ...
        with self.lock:
            ...
            key = self._generate_key("backtest", symbol=symbol, strategy=strategy,
    ...
    def get_backtest_result(self, symbol: str, strategy: str, parameters: Dict[str, Any],
        ...
        with self.lock:
            params_hash = self._hash_parameters(parameters)
            entries = self._find_entries("backtest", symbol=symbol, 
                                       parameters_hash=params_hash, interval=interval)
    ...
    def cache_optimization_result(self, symbol: str, strategy: str, 
        ...
        with self.lock:
            ...
            key = self._generate_key("optimization", symbol=symbol, strategy=strategy,
                                   parameters_hash=config_hash, interval=interval)
    ...
    def get_optimization_result(self, symbol: str, strategy: str,
        ...
        with self.lock:
            config_hash = self._hash_parameters(optimization_config)
            entries = self._find_entries("optimization", symbol=symbol,
                                       parameters_hash=config_hash, interval=interval)
    ...
    def _save_entry(self, entry: CacheEntry, file_path: Path):
        ...
        with sqlite3.connect(self.metadata_db) as conn:
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """, (
                entry.key, entry.cache_type, entry.symbol,
                entry.created_at.isoformat(), entry.last_accessed.isoformat(),
                entry.expires_at.isoformat() if entry.expires_at else None,
                entry.size_bytes, entry.source, entry.interval, entry.data_type,
                entry.parameters_hash, entry.version, str(file_path)
            ))
    ...
    def _find_entries(self, cache_type: str, **filters) -> List[CacheEntry]:
        ...
        with sqlite3.connect(self.metadata_db) as conn:
            cursor = conn.execute(
                f"SELECT * FROM cache_entries WHERE {where_clause}",
                params
            )
    ...
    def _remove_entry(self, key: str):
        ...
        with sqlite3.connect(self.metadata_db) as conn:
            cursor = conn.execute("SELECT cache_type FROM cache_entries WHERE key = ?", (key,))
            row = cursor.fetchone()
    ...
    def _cleanup_expired(self):
        ...
        with sqlite3.connect(self.metadata_db) as conn:
            cursor = conn.execute(
                "SELECT key, cache_type FROM cache_entries WHERE expires_at < ?",
                (now,)
            )
