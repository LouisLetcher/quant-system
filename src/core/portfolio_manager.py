...
class PortfolioManager:
    ...
    def analyze_portfolios(self, portfolios: Dict[str, List[BacktestResult]]) -> Dict[str, Any]:
        """
        ...
        investment_recommendations = self._generate_investment_recommendations(ranked_portfolios, detailed_analysis)
        
        # Generate overall analysis
        overall_analysis = {
            'analysis_date': datetime.now().isoformat(),
            'portfolios_analyzed': len(portfolios),
            'portfolio_summaries': {name: asdict(summary) for name, summary in portfolio_summaries.items()},
            ...
            'diversification_analysis': self._analyze_diversification_opportunities(portfolios)
        }
        
        return overall_analysis
    
    def generate_investment_plan(self, total_capital: float, 
        ...
        """
        self.logger.info(f"Generating investment plan for ${total_capital:,.2f} with {risk_tolerance} risk tolerance")
        ...
        allocations = self._calculate_capital_allocations(suitable_recommendations, total_capital, risk_tolerance)
        ...
        investment_plan = {
            ...
            'expected_portfolio_metrics': self._calculate_expected_portfolio_metrics(allocations),
    ...
    def _calculate_portfolio_summary(self, name: str, results: List[BacktestResult]) -> PortfolioSummary:
        """Calculate summary statistics for a portfolio."""
        if not results:
            return PortfolioSummary(
                name=name, total_assets=0, total_strategies=0,
                best_performer="N/A", worst_performer="N/A",
                avg_return=0, avg_sharpe=0, max_drawdown=0,
                risk_score=0, return_score=0, overall_score=0,
                investment_priority=999, recommended_allocation=0,
        ...
        if not successful_results:
            return PortfolioSummary(
                name=name, total_assets=len(results), total_strategies=0,
                best_performer="N/A", worst_performer="N/A",
                avg_return=0, avg_sharpe=0, max_drawdown=0,
                risk_score=0, return_score=0, overall_score=0,
                investment_priority=999, recommended_allocation=0,
        ...
        overall_score = (return_score * 0.6) + (risk_score * 0.4)  # Weight returns higher
        
        # Determine risk category
        risk_category = self._determine_risk_category(risk_score, np.mean(drawdowns), np.std(returns))
    ...
    def _calculate_detailed_metrics(self, results: List[BacktestResult]) -> Dict[str, Any]:
        ...
        for strategy in set(r.strategy for r in successful_results):
            ...
            strategy_returns = [r.metrics.get('total_return', 0) for r in strategy_results]
            
            strategy_performance[strategy] = {
                'count': len(strategy_results),
                'avg_return': np.mean(strategy_returns),
                'success_rate': len([r for r in strategy_returns if r > 0]) / len(strategy_returns) * 100,
        ...
        for symbol in set(r.symbol for r in successful_results):
            ...
            asset_performance[symbol] = {
                ...
                'consistency': 1 - (np.std(symbol_returns) / np.mean(symbol_returns)) if np.mean(symbol_returns) != 0 else 0,
                'best_strategy': max(symbol_results, key=lambda x: x.metrics.get('total_return', 0)).strategy
            }
    ...
    def _rank_portfolios(self, summaries: Dict[str, PortfolioSummary]) -> List[Tuple[str, PortfolioSummary]]:
        """Rank portfolios by overall score."""
        # Sort by overall score (descending)
        ranked = sorted(summaries.items(), key=lambda x: x[1].overall_score, reverse=True)
    ...
    def _generate_investment_recommendations(self, ranked_portfolios: List[Tuple[str, PortfolioSummary]],
                                           detailed_analysis: Dict[str, Any]) -> List[InvestmentRecommendation]:
        ...
        for i, (name, summary) in enumerate(ranked_portfolios):
            ...
            recommended_allocation = min(base_allocation * risk_adjustment, 40)  # Cap at 40%
            
            # Generate rationale and key points
            rationale = self._generate_investment_rationale(summary, detailed_analysis.get(name, {}))
            strengths = self._identify_key_strengths(summary, detailed_analysis.get(name, {}))
            risks = self._identify_key_risks(summary, detailed_analysis.get(name, {}))
            
            # Calculate confidence score
            confidence = self._calculate_confidence_score(summary, detailed_analysis.get(name, {}))
            ...
            recommendation = InvestmentRecommendation(
                ...
                expected_volatility=self._estimate_volatility(detailed_analysis.get(name, {})),
    ...
    def _determine_risk_category(self, risk_score: float, avg_drawdown: float, return_volatility: float) -> str:
    ...
    def _generate_market_analysis(self, summaries: Dict[str, PortfolioSummary]) -> Dict[str, Any]:
        ...
        return {
            'market_sentiment': 'Bullish' if np.mean(all_returns) > 5 else 'Bearish' if np.mean(all_returns) < -2 else 'Neutral',
            ...
            'top_performing_category': max(summaries.keys(), key=lambda k: summaries[k].avg_return),
            'most_consistent_category': max(summaries.keys(), key=lambda k: summaries[k].avg_sharpe),
            'recommendations': self._generate_market_recommendations(summaries)
        }
    
    def _generate_risk_analysis(self, summaries: Dict[str, PortfolioSummary]) -> Dict[str, Any]:
    ...
    def _analyze_diversification_opportunities(self, portfolios: Dict[str, List[BacktestResult]]) -> Dict[str, Any]:
        ...
        return {
            ...
            'diversification_opportunities': self._identify_diversification_gaps(portfolio_overlap),
            'recommended_portfolio_mix': self._recommend_portfolio_mix(portfolio_overlap)
        }
    
    def _filter_by_risk_tolerance(self, recommendations: List[Dict], risk_tolerance: str) -> List[Dict]:
        """Filter recommendations based on risk tolerance."""
        risk_mapping = {
        ...
        allowed_categories = risk_mapping.get(risk_tolerance, ['Conservative', 'Moderate'])
        
        return [rec for rec in recommendations if rec['risk_category'] in allowed_categories]
    
    def _calculate_capital_allocations(self, recommendations: List[Dict], 
                                     total_capital: float, risk_tolerance: str) -> List[Dict]:
        ...
        for rec in recommendations:
            adjusted_pct = rec['recommended_allocation_pct'] * multipliers.get(rec['risk_category'], 1.0)
    ...
    def _generate_risk_management_plan(self, allocations: List[Dict], analysis: Dict[str, Any]) -> Dict[str, Any]:
        """Generate risk management plan."""
        total_allocation = sum(a['allocation_amount'] for a in allocations)
        
        # Calculate portfolio risk metrics
        weighted_return = sum(a['expected_return'] * a['allocation_percentage'] / 100 for a in allocations)
    ...
    def _calculate_expected_portfolio_metrics(self, allocations: List[Dict]) -> Dict[str, float]:
        ...
        portfolio_volatility = np.sqrt(sum(w**2 * (r * 0.5)**2 for w, r in zip(weights, returns)))
        
        return {
            'expected_annual_return': expected_return,
            'expected_volatility': portfolio_volatility,
            'expected_sharpe_ratio': expected_return / portfolio_volatility if portfolio_volatility > 0 else 0,
    ...
    def _get_risk_adjustment(self, risk_category: str) -> float:
        ...
        return {'Conservative': 1.2, 'Moderate': 1.0, 'Aggressive': 0.8}.get(risk_category, 1.0)
    ...
    def _generate_investment_rationale(self, summary: PortfolioSummary, detailed_analysis: Dict) -> str:
    ...
    def _identify_key_strengths(self, summary: PortfolioSummary, detailed_analysis: Dict) -> List[str]:
        ...
        if summary.avg_sharpe > 1:
            strengths.append(f"Strong risk-adjusted returns (Sharpe: {summary.avg_sharpe:.2f})")
    ...
    def _identify_key_risks(self, summary: PortfolioSummary, detailed_analysis: Dict) -> List[str]:
        """Identify key risks."""
        risks = []
    ...
    def _calculate_confidence_score(self, summary: PortfolioSummary, detailed_analysis: Dict) -> float:
        """Calculate confidence score."""
    ...
    def _generate_risk_recommendations(self, risk_analysis: Dict) -> List[str]:
        return ["Maintain diversification", "Monitor correlation changes", "Review risk limits regularly"]
    
    def _classify_asset_types(self, symbols: set) -> Dict[str, int]:
        crypto_count = len([s for s in symbols if any(c in s.upper() for c in ['BTC', 'ETH', 'USD', 'USDT'])])
    ...
    def _identify_diversification_gaps(self, portfolio_overlap: Dict) -> List[str]:
        return ["Consider adding international exposure", "Evaluate sector concentration"]
