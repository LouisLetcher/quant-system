...
class UnifiedResultAnalyzer:
    ...
    def calculate_metrics(self, backtest_result: Dict[str, Any], 
        ...
        try:
            ...
            metrics = {
                ...
                'annualized_return': self._calculate_annualized_return(equity_values, initial_capital),
                'volatility': self._calculate_volatility(returns),
                'sharpe_ratio': self._calculate_sharpe_ratio(returns),
                'sortino_ratio': self._calculate_sortino_ratio(returns),
                'calmar_ratio': self._calculate_calmar_ratio(equity_values, initial_capital),
                'max_drawdown': self._calculate_max_drawdown(equity_values),
                'max_drawdown_duration': self._calculate_max_drawdown_duration(equity_values),
    ...
    def calculate_optimization_metrics(self, optimization_results: Dict[str, Any]) -> Dict[str, float]:
        """
        ...
        try:
            ...
            best_scores = [entry.get('best_score', 0) for entry in history if 'best_score' in entry]
    ...
    def _calculate_trade_metrics(self, trades: pd.DataFrame) -> Dict[str, float]:
        ...
        if trades.empty:
            return {
                ...
                'largest_win': 0, 'largest_loss': 0, 'num_trades': 0,
                'avg_trade_duration': 0, 'expectancy': 0
            }
        
        # Filter trades with PnL information
        trades_with_pnl = trades[trades['pnl'] != 0] if 'pnl' in trades.columns else pd.DataFrame()
        
        if trades_with_pnl.empty:
            return {
                'win_rate': 0, 'profit_factor': 0, 'avg_win': 0, 'avg_loss': 0,
                'largest_win': 0, 'largest_loss': 0, 'num_trades': len(trades),
                'avg_trade_duration': 0, 'expectancy': 0
            }
