...
class DataManager:
    ...
    @staticmethod
    def get_stock_data(
        ticker, start_date=None, end_date=None, interval="1d", use_cache=True
    ):
        """
        ...
        if use_cache:
            ...
            if cached_data is not None and not cached_data.empty:
                # Determine appropriate recency threshold based on interval
                recency_thresholds = {
                    "1d": timedelta(days=2),
        ...
        try:
            ...
            if start is None:
                ...
                data = Scraper.fetch_data(
                    ticker, period=period, end=end_date, interval=interval
                )
            else:
            ...
            print(f"❌ Error fetching data for {ticker}: {e!s}")
            return None
