...
class AdvancedReportGenerator:
    ...
    def generate_portfolio_report(self, results: List[BacktestResult], 
        ...
        # Check cache
        cache_key = self._get_report_cache_key("portfolio", results, title, include_charts, format)
        ...
        if format == "html":
            report_path = self._generate_html_portfolio_report(report_data, charts, title)
        elif format == "json":
        ...
        self.logger.info(f"Portfolio report generated in {generation_time:.2f}s: {report_path}")
        
        return str(report_path)
    
    def generate_strategy_comparison_report(self, results: Dict[str, List[BacktestResult]],
        ...
        # Check cache
        cache_key = self._get_report_cache_key("strategy_comparison", results, title, include_charts, format)
        if self.cache_reports:
            cached_report = self._get_cached_report(cache_key)
            if cached_report:
        ...
        self.logger.info(f"Generating strategy comparison report for {len(results)} strategies")
        ...
        # Generate report
        if format == "html":
            report_path = self._generate_html_strategy_comparison_report(comparison_data, charts, title)
        elif format == "json":
            report_path = self._generate_json_strategy_comparison_report(comparison_data, title)
        else:
            raise ValueError(f"Unsupported format: {format}")
        ...
        generation_time = time.time() - start_time
        self.logger.info(f"Strategy comparison report generated in {generation_time:.2f}s: {report_path}")
        
        return str(report_path)
    
    def generate_optimization_report(self, optimization_results: Dict[str, Dict[str, OptimizationResult]],
        ...
        # Check cache
        cache_key = self._get_report_cache_key("optimization", optimization_results, title, include_charts, format)
        if self.cache_reports:
            cached_report = self._get_cached_report(cache_key)
            if cached_report:
        ...
        # Generate report
        if format == "html":
            report_path = self._generate_html_optimization_report(optimization_data, charts, title)
        elif format == "json":
            report_path = self._generate_json_optimization_report(optimization_data, title)
        else:
            raise ValueError(f"Unsupported format: {format}")
        ...
        generation_time = time.time() - start_time
        self.logger.info(f"Optimization report generated in {generation_time:.2f}s: {report_path}")
    ...
    def _prepare_portfolio_data(self, results: List[BacktestResult]) -> Dict[str, Any]:
        ...
        portfolio_stats = {
            ...
            'successful_backtests': len(df[df['total_return'] > 0]) if not df.empty else 0,
            'avg_return': df['total_return'].mean() if not df.empty else 0,
            'avg_sharpe': df['sharpe_ratio'].mean() if not df.empty else 0,
            'best_strategy': df.loc[df['total_return'].idxmax(), 'strategy'] if not df.empty else None,
            'best_symbol': df.loc[df['total_return'].idxmax(), 'symbol'] if not df.empty else None,
        ...
        if not df.empty:
            for strategy in df['strategy'].unique():
                ...
                strategy_performance[strategy] = {
                    ...
                    'win_rate': len(strategy_df[strategy_df['total_return'] > 0]) / len(strategy_df) * 100,
                    'best_symbol': strategy_df.loc[strategy_df['total_return'].idxmax(), 'symbol']
                }
        ...
        if not df.empty:
            for symbol in df['symbol'].unique():
                ...
                symbol_performance[symbol] = {
                    ...
                    'best_strategy': symbol_df.loc[symbol_df['total_return'].idxmax(), 'strategy']
                }
    ...
    def _prepare_strategy_comparison_data(self, results: Dict[str, List[BacktestResult]]) -> Dict[str, Any]:
        ...
        for strategy, strategy_results in results.items():
            ...
            if strategy_metrics:
                comparison_stats[strategy] = {
                    ...
                    'avg_return': np.mean([m.get('total_return', 0) for m in strategy_metrics]),
                    'std_return': np.std([m.get('total_return', 0) for m in strategy_metrics]),
                    'avg_sharpe': np.mean([m.get('sharpe_ratio', 0) for m in strategy_metrics]),
                    'avg_drawdown': np.mean([m.get('max_drawdown', 0) for m in strategy_metrics]),
                    'win_rate': np.mean([m.get('win_rate', 0) for m in strategy_metrics]),
                    'best_return': max([m.get('total_return', 0) for m in strategy_metrics]),
                    'worst_return': min([m.get('total_return', 0) for m in strategy_metrics])
                }
    ...
    def _prepare_optimization_data(self, optimization_results: Dict[str, Dict[str, OptimizationResult]]) -> Dict[str, Any]:
    ...
    def _generate_portfolio_charts(self, data: Dict[str, Any]) -> Dict[str, str]:
        ...
        fig_returns = px.histogram(df, x='total_return', nbins=30, 
                                 title='Distribution of Returns')
        fig_returns.update_layout(xaxis_title='Total Return (%)', yaxis_title='Frequency')
        ...
        # Risk-Return scatter
        fig_scatter = px.scatter(df, x='max_drawdown', y='total_return', 
                               color='strategy', size='sharpe_ratio',
                               hover_data=['symbol'],
        ...
        top_performers = df.nlargest(10, 'total_return')[['symbol', 'strategy', 'total_return', 'sharpe_ratio']]
        fig_table = go.Figure(data=[go.Table(
            header=dict(values=['Symbol', 'Strategy', 'Return (%)', 'Sharpe Ratio'],
                       fill_color='paleturquoise',
                       align='left'),
            cells=dict(values=[top_performers.symbol, top_performers.strategy,
    ...
    def _generate_strategy_comparison_charts(self, data: Dict[str, Any]) -> Dict[str, str]:
    ...
    def _generate_optimization_charts(self, data: Dict[str, Any]) -> Dict[str, str]:
        ...
        if not convergence_df.empty:
            fig_convergence = px.line(convergence_df, x='generation', y='best_score',
                                    color='key', title='Optimization Convergence')
    ...
    def _generate_html_portfolio_report(self, data: Dict[str, Any], 
                                      charts: Dict[str, str], title: str) -> Path:
    ...
    def _generate_html_strategy_comparison_report(self, data: Dict[str, Any],
                                                charts: Dict[str, str], title: str) -> Path:
    ...
    def _generate_html_optimization_report(self, data: Dict[str, Any],
                                         charts: Dict[str, str], title: str) -> Path:
    ...
    def _generate_json_strategy_comparison_report(self, data: Dict[str, Any], title: str) -> Path:
    ...
    def _generate_json_optimization_report(self, data: Dict[str, Any], title: str) -> Path:
...
<body>
    ...
    {% endfor %}
        ...
        strategy_template = portfolio_template.replace("{{ title }}", "Strategy Comparison Report")
        ...
        optimization_template = portfolio_template.replace("{{ title }}", "Optimization Analysis Report")
...
class ReportScheduler:
    ...
    def schedule_daily_portfolio_report(self, results_function: Callable, 
                                      title: str = "Daily Portfolio Report"):
        """Schedule daily portfolio report generation."""
        # Implementation for scheduling would go here
        pass
    
    def schedule_weekly_optimization_report(self, optimization_function: Callable,
                                          title: str = "Weekly Optimization Report"):
