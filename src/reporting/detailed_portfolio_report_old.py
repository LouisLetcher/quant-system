...
class DetailedPortfolioReporter:
    ...
    def _analyze_asset_with_timeframes(
        ...
        try:
            # Get the best strategy for this symbol from database
            best_strategy_db = (
            ...
            if best_strategy_db:
                # Use database best strategy
                best_strategy = best_strategy_db.strategy
                best_timeframe = best_strategy_db.timeframe
                best_metrics = {
                    "sharpe_ratio": float(best_strategy_db.sharpe_ratio or 0),
                    "sortino_ratio": float(best_strategy_db.sortino_ratio or 0),
                    "calmar_ratio": float(best_strategy_db.calmar_ratio or 0),
                    "total_return": float(best_strategy_db.total_return or 0),
                    "max_drawdown": float(best_strategy_db.max_drawdown or 0),
                    "win_rate": 0.0,  # Not stored in BestStrategy table
                    "volatility": 0.0,  # Not stored in BestStrategy table
                    "num_trades": 0,  # Not stored in BestStrategy table
                }
            else:
                # Fallback if not in database
                best_strategy = strategies[0]
                best_timeframe = timeframes[0]
                best_metrics = self._simulate_strategy_timeframe_performance(
                    symbol, best_strategy, best_timeframe
                )

            best_combination = {
                "strategy": best_strategy,
                "timeframe": best_timeframe,
                "score": best_metrics["sortino_ratio"],
    ...
    def _simulate_strategy_timeframe_performance(
        ...
        """Get real strategy+timeframe performance from database or direct backtest."""
        from sqlalchemy.orm import sessionmaker
        ...
        try:
            # Look for existing backtest result
            result = (
                session.query(BacktestResult)
                .filter_by(symbols=[symbol], strategy=strategy, timeframe=timeframe)
                .first()
            )
            ...
            if result:
                return {
                    ...
                    "num_trades": int(result.trades_count or 0),
                }
            # Fallback to direct backtest if not in database

            from src.core.direct_backtest import run_direct_backtest

            backtest_result = run_direct_backtest(
                symbol=symbol,
            ...
            print(f"Error getting real data for {symbol}/{strategy}/{timeframe}: {e}")
            # Return minimal defaults if everything fails
            return {
                "sharpe_ratio": 0.0,
                "sortino_ratio": 0.0,
                "calmar_ratio": 0.0,
    ...
    def _generate_detailed_metrics_with_timeframe(
        ...
        """Generate detailed metrics using real database data."""
        from sqlalchemy.orm import sessionmaker
        ...
        try:
            ...
            # Convert trades to orders format for display
            orders = []
            for trade in trades:
                orders.append(
                    {
            ...
            return {
                "overview": {
                    "PSR": metrics.get("sortino_ratio", 0),
                    "sharpe_ratio": metrics.get("sharpe_ratio", 0),
                    "total_orders": len(orders),
                    "net_profit": metrics.get("total_return", 0) * 100,
                    "average_win": metrics.get("average_win", 0) * 100,
                    "average_loss": abs(metrics.get("average_loss", 0)) * 100,
            ...
            print(f"Error generating real metrics for {symbol}/{strategy}: {e}")
            return {
                "overview": {
                    "PSR": 0,
                    "sharpe_ratio": 0,
