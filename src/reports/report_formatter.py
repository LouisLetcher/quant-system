...
class ReportFormatter:
    ...
    def format_backtest_results(results):
        ...
        return {
            ...
            "max_drawdown": (
                ...
                if isinstance(results.get("max_drawdown", 0), float)
                else results.get("max_drawdown", "0.00%")
            ),
        }
    ...
    def format_multiasset_results(results_dict, metric="sharpe_ratio"):
        ...
        is_multi_strategy = (
            "is_multi_strategy" in next(iter(results_dict.values()))
            if results_dict
            else False
        )
        ...
        for name, result in results_dict.items():
            ...
            if metric == "sharpe_ratio":
                ...
                score = (
                    float(return_str.replace("%", ""))
                    if isinstance(return_str, str)
                    else result.get("return_pct", 0)
                )
            else:
            ...
            if is_multi_strategy:
                formatted_data["strategies"][name] = (
                    ReportFormatter.format_backtest_results(result)
                )
            else:
                formatted_data["assets"][name] = (
                    ReportFormatter.format_backtest_results(result)
                )

        # Sort comparison by score
        formatted_data["comparison"] = sorted(
            formatted_data["comparison"], key=lambda x: x["score"], reverse=True
        )

        # Add metadata
        formatted_data["is_multi_strategy"] = is_multi_strategy
        formatted_data["is_multi_asset"] = not is_multi_strategy
        formatted_data["best_name"] = (
            formatted_data["comparison"][0]["name"]
            if formatted_data["comparison"]
            else None
        )
        formatted_data["best_score"] = (
            formatted_data["comparison"][0]["score"]
            if formatted_data["comparison"]
            else 0
        )
...
class ReportFormatter:
    ...
    def prepare_portfolio_report_data(portfolio_results):
        ...
        report_data = {
            ...
            "date_generated": datetime.now(tz=timezone.utc).strftime(
                "%Y-%m-%d %H:%M:%S"
            ),
            "assets": [],
            "summary": {
        ...
        asset_data = portfolio_results.get(
            "best_combinations", portfolio_results.get("assets", {})
        )
                        ...
                        equity_curve.append(
                            {
                                "date": str(date),
                                "value": float(
                                    row.iloc[0] if isinstance(row, pd.Series) else row
                                ),
                            }
                        )
        ...
        if report_data["assets"]:
            report_data["summary"]["total_return"] = total_return_pct / len(
                report_data["assets"]
            )
