...
class TestParameterOptimizer(unittest.TestCase):
    ...
    def test_optimize_portfolio_parameters(self, mock_get_config, mock_extract):
        ...
        with patch("src.portfolio.parameter_optimizer.DataLoader"):
            with patch("src.portfolio.parameter_optimizer.ParameterTuner"):
                with patch(
                    "src.portfolio.parameter_optimizer.run_backtest_with_params"
                ):
                    with patch(
                        "src.portfolio.parameter_optimizer.extract_detailed_metrics"
                    ):
                        with patch(
                            "src.portfolio.parameter_optimizer.generate_equity_chart"
                        ):
                            with patch(
                                "src.portfolio.parameter_optimizer.ReportGenerator"
                            ):
                                # Call function
    ...
    def test_get_param_ranges(self):
        ...
        class TestStrategy:
            param_ranges = {"sma_period": (10, 50), "std_dev": (1.0, 3.0)}
        ...
        self.assertTrue(
            0 < result["threshold"][0] < result["threshold"][1] < 1
