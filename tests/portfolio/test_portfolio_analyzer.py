...
class TestPortfolioAnalyzer(unittest.TestCase):
    ...
    def test_run_strategy_on_asset(self, mock_backtest_engine, mock_data_loader):
        ...
        # Assertions
        mock_data_loader.load_data.assert_called_with('AAPL', period='1mo', interval='1d')
    ...
    def test_find_best_strategy_for_asset(self, mock_run_strategy):
        ...
        mock_run_strategy.side_effect = [
            {'metrics': {'sharpe_ratio': 1.2}, 'strategy': 'mean_reversion', 'interval': '1d'},
            {'metrics': {'sharpe_ratio': 1.5}, 'strategy': 'momentum', 'interval': '1d'},
            {'metrics': {'sharpe_ratio': 1.0}, 'strategy': 'breakout', 'interval': '1d'}
        ]
