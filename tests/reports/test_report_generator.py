...
class TestReportGenerator(unittest.TestCase):
    ...
    def test_prepare_template_variables(self):
        ...
        result = self.generator._prepare_template_variables(
            test_data_with_nan, 
            self.generator.TEMPLATES["single_strategy"]
        )
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('src.reports.report_generator.Environment')
    def test_generate_parameter_optimization_report(self, mock_env, mock_file, mock_makedirs):
        # Setup mock template
        mock_template = MagicMock()
        mock_env_instance = MagicMock()
        mock_env.return_value = mock_env_instance
        mock_env_instance.get_template.return_value = mock_template
        mock_template.render.return_value = '<html>Test Parameter Optimization Report</html>'
        ...
        # Call method
        output_path = self.generator.generate_parameter_optimization_report(optimization_data)
        
        # Assertions
        mock_env_instance.get_template.assert_called_with("parameter_optimization_report.html")
        mock_template.render.assert_called()
        mock_file.assert_called()
        self.assertTrue(output_path.endswith(".html"))
